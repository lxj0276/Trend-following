function[tableD,tableK]=RBreaker_update2(data,paras,lasttime,transactioncost,breakpct,tol)
% break with percentage
[nrow ncol]=size(data);
tick=0.2;
...
for minuteK=days(2):nrow
    ...
    if currenthold==0 %no position
        ...
        if reachSS==0 && reachBS==0
            ...
            elseif buy+sell==2  % worst condition, break with loss
                ...
                points(daycount-1)=(sellprice-buyprice)-(buyprice+sellprice)*transactioncost;
                ...
                %breakprc=tickround(buy*(1-breakpct)*buyprice+sell*(1+breakpct)*sellprice,tick);
    ...
    elseif currenthold==1 %long position, hold the position till a break or endtime
        breakprc=tickround((1-breakpct)*buyprice,tick);
        if time(minuteK)==endtime   %endtime
            ...
            pass=1;
            continue
        elseif (low(minuteK)-breakprc<=tol)   %break
            ind=open(minuteK)<breakprc;
            breakprc=ind*open(minuteK)+(1-ind)*breakprc;
            returns(daycount-1)=(breakprc/buyprice-1)-2*transactioncost;
            points(daycount-1)=(breakprc-buyprice)-(breakprc+buyprice)*transactioncost;
            direction(daycount-1)=2;
        ...
        breakprc=tickround(sell*(1+breakpct)*sellprice,tick);
        if time(minuteK)==endtime  %endtime
            ...
            pass=1;
            continue
        elseif (high(minuteK)-breakprc>=-tol)  %break
            ind=open(minuteK)>breakprc;
            breakprc=ind*open(minuteK)+(1-ind)*breakprc;
            returns(daycount-1)=(-breakprc/sellprice+1)-2*transactioncost;
            points(daycount-1)=(sellprice-breakprc)-(breakprc+sellprice)*transactioncost;
            direction(daycount-1)=-2;
            Kreturns(minuteK-days(2)+1)=(-breakprc/close(minuteK-1)+1)-transactioncost;
