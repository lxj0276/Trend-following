function[tableD,tableK]=RBreaker_update1(data,paras,lasttime,transactioncost,tol)
% two different break line
[nrow ncol]=size(data);
tick=0.2;
...
%     end
    ...
    elseif currenthold==1 %long position, hold the position till a break or endtime
        if time(minuteK)==endtime   %endtime
            ...
            pass=1;
            continue
        elseif buyup && (low(minuteK)-SE(daycount-1)<=tol)  %break
            ind=open(minuteK)<SE(daycount-1);
            brkprc=ind*open(minuteK)+(1-ind)*SE(daycount-1);
            ...
            pass=1;
            continue
        elseif buydown && (low(minuteK)-SB(daycount-1)<=tol)   %break
        ...
        if time(minuteK)==endtime  %endtime
            ...
            pass=1;
            continue
        elseif selldown && (high(minuteK)-BE(daycount-1)>=-tol)  %break
            ind=open(minuteK)>BE(daycount-1);
            brkprc=ind*open(minuteK)+(1-ind)*BE(daycount-1);
            ...
            pass=1;
            continue
        elseif sellup && (high(minuteK)-BB(daycount-1)>=-tol)  %break
